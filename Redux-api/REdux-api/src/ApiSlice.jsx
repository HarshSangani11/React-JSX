...
export const fetchApi=createAsyncThunk("fetchApi",async()=>{
    let response=await axios.get("http://localhost:2125/post");
    return response.data;
})
export const addData = createAsyncThunk("addData", async (newData) => {
    let response = await axios.post("http://localhost:2125/post", newData);
    return response.data;
})

export const deleteData = createAsyncThunk("deleteData", async (i) => {
    await axios.delete(`http://localhost:2125/post/${i}`)
    return i;
})
export const updateData = createAsyncThunk("updateData", async ({ editId, updData }) => {
    await axios.put(`http://localhost:2125/post/${editId}`, updData)
